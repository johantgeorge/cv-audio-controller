        ...
        while True:
            ...
            if result.hand_landmarks:
                for hand_idx, landmarks in enumerate(result.hand_landmarks):
                    ...
                    hand_center_x = get_hand_center_x(landmarks)

                    if static_label == "Open Palm":
                      wave_x_buffers[hand_name].append(hand_center_x)

                      if is_wave_motion(wave_x_buffers[hand_name]):
                          prediction_buffers[hand_name].append("Wave")
                      else:
                          prediction_buffers[hand_name].append(static_label)
                    else:
            ...
            for hand_name in ("Left", "Right"):
                if hand_name not in detected_hands:
                    wave_x_buffers[hand_name].clear()
                    prediction_buffers[hand_name].append(NO_GESTURE)
                    current_labels[hand_name] = smooth_prediction(
                        prediction_buffers[hand_name]
                    )
            ...
            # uncommenting this will create connections between the right and left hand
            ...
            # cv2.putText(
            #     frame,
            #     f"Left: {current_labels['Left']} ({current_scores['Left']:.2f})",
            #     (20, 40),
            #     cv2.FONT_HERSHEY_SIMPLEX,
            #     0.75,
            #     (255, 255, 255),
            ...
            # cv2.putText(
            #     frame,
            #     f"Right: {current_labels['Right']} ({current_scores['Right']:.2f})",
            #     (20, 75),
            #     cv2.FONT_HERSHEY_SIMPLEX,
            #     0.75,
            #     (255, 255, 255),
            ...
            cv2.imshow("Live Gesture Classifier", frame)
